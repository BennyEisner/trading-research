...
class TestTemporalDataLeakageFixes:
    ...
    def test_temporal_sequence_alignment_prevents_leakage(self, extended_ohlcv_data):
        ...
        feature_columns = [
            col for col in mock_features.columns if col not in ["date", "open", "high", "low", "close", "volume"]
        ]

        X, y = trainer._prepare_pattern_detection_sequences(
            mock_features, feature_columns, pattern_targets, sequence_length=20, stride=5
        )
        ...
        for i in range(len(X)):
            ...
            assert (
                target_day > sequence_end_day + temporal_gap
    ...
    def test_configuration_override_functionality(self):
        ...
        assert (
            trainer.config.model.model_params.get("learning_rate") == original_lr
