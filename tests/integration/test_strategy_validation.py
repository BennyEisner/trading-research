...
class TestStrategyValidation:
    ...
    def strategy_validator(self):
        ...
        return create_strategy_validator(
            min_backtest_days=200, max_drawdown_threshold=0.15, min_sharpe_ratio=0.5  # Lower for testing
        )
    ...
    def test_strategy_data_compatibility_validation_success(
        self, engineered_features_data, strategy_validator, rsi_strategy
    ):
    ...
    def test_strategy_data_compatibility_validation_failure(self, strategy_validator, rsi_strategy):
        ...
        insufficient_data = pd.DataFrame({"close": [100, 101, 102], "volume": [1000, 1100, 1200]})

        is_valid, issues = strategy_validator.validate_strategy_data_compatibility(insufficient_data, rsi_strategy)

        assert not is_valid
        assert len(issues) > 0
        assert any("validation failed" in issue.lower() for issue in issues)

    def test_strategy_signal_validation_success(self, engineered_features_data, strategy_validator, rsi_strategy):
        """Test successful strategy signal validation"""

        is_valid, issues = strategy_validator.validate_strategy_signals(engineered_features_data, rsi_strategy)
    ...
    def test_strategy_signal_validation_with_mock_strategy(self, engineered_features_data, strategy_validator):
        ...
        invalid_signals = pd.DataFrame(
            {"position": [2.0, -2.0, 0.5, -0.5, 0.0], "entry_price": [100, 101, 102, 103, 104]}  # Invalid values
        )
        mock_strategy.generate_signals.return_value = invalid_signals

        is_valid, issues = strategy_validator.validate_strategy_signals(engineered_features_data, mock_strategy)

        assert not is_valid
        assert len(issues) > 0
        assert any("invalid position values" in issue.lower() for issue in issues)

    def test_ensemble_strategies_validation_success(
        self, engineered_features_data, strategy_validator, rsi_strategy, macd_strategy
    ):
        """Test successful ensemble strategies validation"""

        strategies = [rsi_strategy, macd_strategy]

        is_valid, issues = strategy_validator.validate_ensemble_strategies(engineered_features_data, strategies)
    ...
    def test_ensemble_strategies_validation_insufficient_strategies(
        self, engineered_features_data, strategy_validator, rsi_strategy
    ):
    ...
    def test_ensemble_graceful_degradation(self, engineered_features_data, strategy_validator, rsi_strategy):
    ...
    def test_backtest_readiness_validation_success(
        self, engineered_features_data, strategy_validator, rsi_strategy, macd_strategy
    ):
        ...
        is_valid, issues = strategy_validator.validate_backtest_readiness(engineered_features_data, strategies)
    ...
    def test_backtest_readiness_validation_insufficient_data(self, strategy_validator, rsi_strategy):
        ...
        is_valid, issues = strategy_validator.validate_backtest_readiness(short_data, [rsi_strategy])
    ...
    def test_time_series_splits_creation(self, engineered_features_data, strategy_validator):
        ...
        cv = strategy_validator.create_time_series_splits(
            engineered_features_data, n_splits=3, test_size=0.2, gap_size=5
        )
    ...
    def test_validation_summary_integration(
        self, engineered_features_data, strategy_validator, rsi_strategy, macd_strategy
    ):
