...
class TrainingDiagnostics:
    ...
    def check_target_construction(self, ticker_data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        for ticker, data in ticker_data.items():
            ...
            if abs(stats['mean']) > 0.01:
                ...
                print(f"   ⚠️  WARNING: Mean return {stats['mean']:.6f} is unusually high")
            
            if stats['std'] > 0.1:
                results['issues'].append(f"{ticker}: Volatility {stats['std']:.6f} seems high")
                print(f"   ⚠️  WARNING: High volatility {stats['std']:.6f}")
            
            if stats['nan_count'] > 0:
                results['issues'].append(f"{ticker}: {stats['nan_count']} NaN values in targets")
                print(f"   ⚠️  WARNING: {stats['nan_count']} NaN values in targets")
            ...
            if 'close' in data.columns:
                ...
                if correlation < 0.99:
                    results['issues'].append(f"{ticker}: daily_return doesn't match price changes (corr={correlation:.3f})")
                    print(f"   ⚠️  WARNING: daily_return correlation with price changes: {correlation:.3f}")
                else:
        ...
        if max(means) - min(means) > 0.005:
            ...
            print(f"   ⚠️  WARNING: Large spread in mean returns across tickers")
    ...
    def check_feature_scaling(self, X_train: np.ndarray, feature_names: List[str] = None) -> Dict:
        ...
        for feature_idx in range(n_features):
            ...
            if abs(stats['min']) > 10.0 or abs(stats['max']) > 10.0:
                results['issues'].append(f"{feature_name}: Range [{stats['min']:.3f}, {stats['max']:.3f}] suggests poor normalization")
    ...
    def analyze_prediction_behavior(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        if pred_stats['std'] < true_stats['std'] * 0.1:
            ...
            print(f"   ⚠️  WARNING: Model predictions have very low variance ({pred_stats['std']:.6f})")
        
        if pred_stats['std'] > true_stats['std'] * 3.0:
            results['issues'].append("Model predictions have excessive variance")
            print(f"   ⚠️  WARNING: Model predictions have excessive variance ({pred_stats['std']:.6f})")
        ...
        if abs(pred_stats['mean'] - true_stats['mean']) > abs(true_stats['mean']) * 2:
            ...
            print(f"   ⚠️  WARNING: Prediction mean ({pred_stats['mean']:.6f}) differs significantly from true mean ({true_stats['mean']:.6f})")
        ...
        try:
            ...
            if np.isnan(correlation):
                ...
                print(f"   ⚠️  WARNING: Correlation is NaN")
            elif correlation < 0.01:
                results['issues'].append(f"Very low correlation ({correlation:.6f})")
                print(f"   ⚠️  WARNING: Very low correlation ({correlation:.6f})")
        except:
        ...
        if direction_accuracy < 0.52:
            ...
            print(f"   ⚠️  WARNING: Directional accuracy barely better than random")
    ...
    def comprehensive_training_check(self, ticker_data: Dict[str, pd.DataFrame], 
