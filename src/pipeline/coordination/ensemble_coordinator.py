...
class EnsembleCoordinator:
    ...
    def coordinate_ensemble_execution(
        self, data: pd.DataFrame, strategies: List[BaseStrategy]
    ) -> Tuple[List[BaseStrategy], Dict[str, Any]]:
        """
        ...
        for strategy in strategies:
            ...
            if is_compatible:
                ...
                strategy_validations[strategy.name] = {"validation_passed": True, "issues": issues}
            else:
                coordination_results["invalid_strategies"].append(
                    {"strategy_name": strategy.name, "error": "; ".join(issues)}
                )
                strategy_validations[strategy.name] = {"validation_passed": False, "error": "; ".join(issues)}
