...
from .utils import DataValidator, FeatureScaler, clean_financial_data, prepare_lstm_sequences
...
class FeatureEngineer:
    ...
    def calculate_all_features(
        self, data: pd.DataFrame, validate_data: bool = True, clean_data: bool = True
    ) -> pd.DataFrame:
        """
    ...
    def select_best_features(
        self, data: pd.DataFrame, target_col: str = "daily_return", max_features: int = 50, method: str = "ensemble"
    ) -> List[str]:
        """
        ...
        if method == "ensemble":
            selected_features = self.feature_selector.select_features(X, y, candidate_features, max_features)
            ...
            from .selectors import ModelBasedFeatureSelector, StatisticalFeatureSelector
    ...
    def prepare_for_lstm(
        ...
        lstm_data = data.copy()
        if scale_features:
            lstm_data = self.feature_scaler.fit_transform(lstm_data, method="robust", exclude_features=[target_col])

        # Create sequences
        X, y = prepare_lstm_sequences(lstm_data, available_features, target_col, sequence_length)
